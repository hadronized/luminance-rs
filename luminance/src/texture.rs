...
where L: Layerable,
      D: Dimensionable,
      P: Pixel {
  // number of bits in the input texels argument
  let input_bits = texels.len() * mem::size_of::<P::RawEncoding>() * 8;
  let pf = P::pixel_format();
  let expected_bits = D::count(size) * pf.format.size();

  if input_bits < expected_bits {
    // potential segfault / overflow; abort
    return Err(TextureError::NotEnoughPixels(expected_bits, input_bits));
  }
...
pub enum TextureError {
  ...
  ///
  /// The first [`usize`] is the number of expected bits to be uploaded and the second [`usize`] is
  ...
  fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
    match *self {
      ...
      TextureError::NotEnoughPixels(expected, provided) => {
        write!(f, "not enough texels provided: expected {} bits, provided {} bits", expected, provided)
      }
    }
  }
}
